...
def user_info(request):
    try:
        ...
        is_student_profile = bool(profile and profile.role == 'student')
        # LMS-only: student role without staff/superuser flags
        is_student = bool(is_student_profile and not user.is_superuser and not user.is_staff)
        can_access_workspace = bool(user.is_superuser or user.is_staff or not is_student_profile)
...
def update_own_profile(request):
    ...
    try:
            ...
            if field in request.data:
                try:
                    ...
                    if field == 'date_of_birth':
                        ...
                        new_value = request.data[field] or ''
                        current_value = getattr(profile, field, '') or ''
        ...
        # Always save profile if any fields were updated, or if user was updated
        if profile_updated or updated_profile_fields or user.email or user.first_name or user.last_name:
            try:
                # Save with update_fields for efficiency, but fallback to full save if needed
                if updated_profile_fields:
                    profile.save(update_fields=updated_profile_fields)
                else:
                    profile.save()
                logger.info(f"Profile saved successfully for user {user.username}. Updated fields: {updated_profile_fields}")
            except Exception as e:
                logger.error(f"Error saving profile with update_fields: {str(e)}", exc_info=True)
                # Try saving without update_fields as fallback
                try:
                    profile.save()
                    logger.info(f"Profile saved successfully (fallback method) for user {user.username}")
                except Exception as e2:
                    logger.error(f"Error saving profile (fallback also failed): {str(e2)}", exc_info=True)
                    raise
            
            # Refresh from database to ensure we return the actual saved values
            try:
                user.refresh_from_db()
                profile.refresh_from_db()
                logger.info(f"Refreshed user and profile from database for {user.username}")
            except Exception as e:
                logger.warning(f"Error refreshing from database: {str(e)}", exc_info=True)
        ...
        if profile:
            try:
                ...
                logger.info(f"Returning profile data for {user.username}: email={response_data['email']}, first_name={response_data['first_name']}, phone={response_data['phone']}")
            except Exception as e:
...
def send_verification_email_endpoint(request):
    ...
    try:
        ...
        if not frontend_url:
            frontend_url = 'http://localhost:3000' if getattr(settings, 'DEBUG', False) else 'https://adminrodeo.com'
