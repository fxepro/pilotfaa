...
export async function POST(request: NextRequest) {
  try {
    ...
    if (domain.startsWith('test-')) {
      ...
      console.log(`[SSL API] TEST MODE: Simulating ${errorType} error`)
    ...
    // Check if DNS lookup failed
    if ((dnsInfo as any).dnsError) {
      return NextResponse.json({
        success: false,
        code: 'DNS_NOT_FOUND',
    ...
    const errorMessage = error instanceof Error ? error.message : "Failed to check SSL and domain information"
    return NextResponse.json(
      { error: errorMessage },
      { status: 500 }
    )
...
async function checkDnsRecords(domain: string) {
  ...
  try {
    ...
    try {
      ...
      if (error.code === 'ENOTFOUND' && !dnsError) {
        dnsError = error
      }
  ...
  // If we got ENOTFOUND and have no records at all, the domain doesn't exist
  const hasAnyRecords = results.ipv4.length > 0 || 
                        results.ipv6.length > 0 || 
                        results.mx.length > 0 ||
                        results.txt.length > 0 ||
                        results.ns.length > 0

  if (dnsError && !hasAnyRecords) {
    // Return as handled error, not throwable
    results.dnsError = {
      code: 'DNS_NOT_FOUND',
      message: `Domain ${domain} cannot be resolved`
    }
